...
class MCPServer {
  constructor(name) {
    ...
    // Setup the main endpoint for tool execution
    this.app.post('/run-tool', async (req, res) => {
      ...
      try {
        ...
        const result = await this.tools[toolName]({ ...params, ctx });
...
mcp.tool("Search DuckDuckGo and return formatted results")(
  async function search({ query, maxResults = 10, ctx }) {
    try {
      ...
      return searcher.formatResultsForLLM(results);
    } catch (error) {
      console.error(error);
